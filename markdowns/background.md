# Background: Observational Cosmology Primer
...
$$
f_\text{obs} = f_\text{0} \left (\frac{1}{1 -v_\text{source} / v_\text{wave}}  \right )
$$
{#eq:ambdop}

where we have simplified the equation in terms of the ratio of the source velocity to the speed of the wave.
...
where we have expressed the relative velocity $v$ (in \si{km\per\s}) in terms of the low-velocity limit from @eq:lowvelocity, $H_0$ is the proportionality constant between the relative velocity and the distance to the object in \si{\km\per\s\per\Mpc}, and $d$ is the distance in \si{Mpc}. The proportionality constant $H_0$ is better known as the Hubble constant, but is most usefully written as $H_0= 100\, h$\si{\km\per\s\per\Mpc} where h is the dimensionless Hubble parameter defined by the current accepted value.
